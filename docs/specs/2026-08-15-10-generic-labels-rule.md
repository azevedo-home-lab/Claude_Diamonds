...
same day by explicit Owner order (PII removal); the two standing rules
await the Owner's decision per the CLAUDE.md spec process.
...
over-interpreted a literal UI instruction (hub top bar, PR #33).
...
The "State discipline" section added to CLAUDE.md in this PR was an
explicit Owner order (spec-9 precedent for same-day execution) and is
recorded here for the trail, not proposed for decision.
