...
## State discipline (added 2026-08-15, Owner order, after two
stranded-commit incidents — see PR #32's and PR #34's process notes)

- Verify state live before every mutation; never act on remembered
  state. Concretely: check the PR's open/merged state immediately
  before any push to its branch, and fetch a file's current content
  from the target branch before modifying it. The check precedes the
  push — a check after the push is an incident report, not a check.
